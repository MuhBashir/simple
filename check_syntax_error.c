...
/**
 * repeated_characters -  counts the repetitions of a char
 *
 * @input: input string
 * @i: index
 * Return: repetitions.
...
{
	if (*(input - 1) == *input)
		return (repeated_characters(input - 1, i + 1));
...
int error_sep(char *input, int i, char last)
{
	...
	if (*input == '|')
	{
		...
		if (last == '|')
		{
			count = repeated_characters(input, 0);
			if (count == 0 || count > 1)
				return (i);
		}
	}
...
int check_syntax_err(shell_data *datash, char *input)
{
	...
	if (f_char == -1)
	{
		print_syntax_err(datash, input, begin, 0);
		return (1);
	}
	...
	if (i != 0)
	{
		print_syntax_err(datash, input, begin + i, 1);
		return (1);
	}
